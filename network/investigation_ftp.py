...
from helpers import decode_hex_string_field, ensure_unique_filename, sha256_bytes
...
def extract_ftp_objects_from_pcap(pcap_path: str, outdir: str):
    """ Use tshark --export-objects ftp-data,<outdir> to extract FTP objects as fallback.
    Returns list of pathlib.Path objects of saved files (might be empty). """
...
def extract_tftp_objects_from_pcap(pcap_path: str, outdir: str):
    """ Use tshark --export-objects tftp,<outdir> to extract TFTP objects as fallback.
    Returns list of pathlib.Path objects of saved files (might be empty). """
    outdir_p = Path(outdir)
    outdir_p.mkdir(parents=True, exist_ok=True)
...
def _save_blob(outdir: Path, hint_name: str, data: bytes):
    """ Save data to outdir using ensure_unique_filename; return record dict. """
...
def process_ftp_packet(pkt, host: str, report: dict, totals: dict, saved_files: list, outdir: Path):
    """ Per-packet FTP handling to detect STOR/RETR and opportunistically save payload
    if the packet has data bytes (rare for control packets).
    Appends records to saved_files list and report['suspicious_requests']. """
    try:
...
def parse_tftp_packets_and_save(cap_or_pcap, outdir: Path):
    """ Accept either:
      - a pyshark capture object (iterable of packets) OR
      - a path to a pcap file (string) -> will open pyshark.FileCapture(pcap, display_filter='tftp')
...
def postprocess_ftp_transfers(pcap_path: str, cap, outdir: Path, report: dict,
    ...
    """ Run after main packet loop. Tries:
      - parse_tftp_packets_and_save(cap, outdir)
      - best_effort_uftp_parse(cap, outdir)
      - extract_ftp_objects_from_pcap(pcap_path, outdir/'tshark_ftp_objects') if export_objects
    Appends found files to saved_files and updates totals & report (errors). """
