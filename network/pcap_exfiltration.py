"""
pcap_exfiltration_triage.py

Purpose:
  Triage a PCAP for possible file exfiltration attempts from a given host (IP or MAC).
  - Heuristics: large outbound transfer, many HTTP POST/PUT, FTP STOR, SMB write/create,
    DNS tunneling (long labels / high entropy), suspicious destinations.
  - Attempts to export HTTP/FTP/TFTP objects using tshark from the filtered pcap (host-related traffic).
  - Also attempts to parse multipart/form-data from HTTP requests/responses to preserve original filenames.
  - Includes custom reassembly for UFTP.
  - Produces a CSV log of findings and prints a short report.

Requirements:
  - Python 3.8+
  - tshark available in PATH (Wireshark installation).
  - pip install pyshark python-magic tqdm
    (on Windows use python-magic-bin instead of python-magic)
Usage:
  python pcap_exfiltration_triage.py -p $.pcap file$ -t $source IP-address$ -o $output directory$
"""
...
import tempfile
import csv
from helpers import nice_bytes, sha256_bytes
...
from investigation_http import (
    process_http_packet,
    export_http_objects_from_pcap,
    extract_multipart_file,
)
from investigation_ftp import (
    process_ftp_packet,
    postprocess_ftp_transfers
)
from investigation_smb import (
    process_smb_packet,
    export_smb_objects_from_pcap
)
from investigation_smtp import (
    process_smtp_packet,
    # export_smtp_objects_from_pcap
)
from investigation_dns import (
    process_dns_packet,
    postprocess_dns
)
...
def run_tshark_filter(in_pcap, out_pcap, expression):
    """ Create a filtered pcap containing only traffic where packets match specific expression
    (e.g.) for filtering by host IP or SMB2 "Write Request". """
    cmd = ["tshark", "-r", in_pcap, "-Y", expression, "-w", out_pcap, '-q']
    subprocess.run(cmd, check=True)


def analyze_with_pyshark(pcap_path, host, report, http_objects_outdir: Path,
                         ftp_objects_outdir: Path, smb_objects_outdir: Path, smtp_objects_outdir: Path,
                         export_objects: bool = True):
    """ Streaming analysis using pyshark. Delegates protocol-specific packets parsing to other modules.
    Returns list of saved file description dicts. """
    if pyshark is None:
    ...
    for pkt in tqdm(cap):
        ...
        try:
            if hasattr(pkt, "smb") or hasattr(pkt, "smb2"):
                process_smb_packet(pkt, host, report, totals, large_upload_threshold_bytes)
        except Exception:
            pass

        # --- SMTP: delegate to process_smtp_packet() - investigation_smtp.py module ---
        try:
            # pyshark exposes smtp layer for cleartext SMTP; also check for port-based heuristics
            if hasattr(pkt, "smtp") or (
                    hasattr(pkt, 'tcp') and getattr(pkt.tcp, 'dstport', '') in ['25', '587', '465', '2525']):
                process_smtp_packet(pkt, host, smtp_objects_outdir, report, totals, saved_files)
        except Exception:
            pass

        # ----- DNS: delegate to process_dns_packet() - investigation_dns.py module -----

        try:
            if hasattr(pkt, "dns"):
                process_dns_packet(pkt, host, report, totals)
        except Exception:
            pass
    ...
    # Run DNS postprocessing (ML-based detection)
    try:
        postprocess_dns('input/dns_light_exe.pcap', pcap_path, host, report, totals)
    except Exception as e:
        report.setdefault("errors", []).append(f"postprocess_dns failed: {e}")
...
def write_csv_report(outdir, findings, exported_http_info, parsed_saved_files, exported_ftp_tftp_info, exported_smb_info):
    ...
    with open(csvf, "w", newline='', encoding='utf-8') as cf:
        ...
        # # Exported SMTP objects (tshark)
        # writer.writerow(["Exported SMTP objects (tshark)", "count", len(exported_smtp_info)])
        # for info in exported_smtp_info:
        #     writer.writerow(["ExportedSMTPFile", info.get("filename"),
        #                      f"{info.get('size')} bytes; sha256={info.get('sha256')}; type={info.get('filetype')}"])
...
def main():
    ...
    # Prepare smtp objects dir
    smtp_objects_dir = outdir / "smtp_objects"
    smtp_objects_dir.mkdir(parents=True, exist_ok=True)
    ...
    parsed_saved_files = analyze_with_pyshark(str(filtered_pcap), host, findings, http_objects_dir, ftp_objects_dir,
                                              smb_objects_outdir=smb_objects_dir, smtp_objects_outdir=smtp_objects_dir,
                                              export_objects=not args.no_export)
    ...
    # Exported SMTP objects from tshark (if exported)
    # exported_smtp_info = []
    # if not args.no_export:
    #     print("[*] Attempting to export SMTP objects from filtered pcap (using tshark --export-objects)...")
    #     try:
    #         tshark_smtp_export_dir = smtp_objects_dir / "tshark_smtp_objects"
    #         tshark_smtp_export_dir.mkdir(exist_ok=True)
    #         export_smtp_objects_from_pcap(str(filtered_pcap), str(tshark_smtp_export_dir))
    #         exported_smtp_info = hash_and_describe_files(tshark_smtp_export_dir)
    #         print(f"[*] Exported {len(exported_smtp_info)} SMTP objects to {tshark_smtp_export_dir}")
    #     except Exception as e:
    #         print("[!] SMTP object export failed or produced no objects:", e)
    ...
    write_csv_report(outdir, findings, exported_http_info, parsed_descriptions, exported_ftp_tftp_info,
                     exported_smb_info)  # exported_smtp_info
    ...
    print(f"SMTP frames: {totals.get('smtp_frames', 0)}, SMTP attachments saved: {totals.get('smtp_attachments_saved', 0)}")
    print(f"DNS queries: {totals.get('dns_queries',0)}, DNS suspected tunneling: {totals.get('dns_suspected_tunnel',0)}")
    ...
    # if exported_smtp_info:
    #     print(f"tshark-exported SMTP objects saved to: {smtp_objects_dir / 'tshark_smtp_objects'} (count={len(exported_smtp_info)})")
