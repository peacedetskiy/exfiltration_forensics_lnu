"""
investigation_http.py

HTTP-related helpers for PCAP exfiltration triage.
Includes multipart/form-data parsing, body extraction, and tshark export utilities.
"""
...
# file type detection (shared with main)
try:
    import magic
...
def parse_content_type_for_boundary(ct_header: str | None):
    """ Extract boundary token from Content-Type header. Returns boundary as bytes (no leading --). """
    if not ct_header:
        return None
...
def split_multipart(body: bytes, boundary: bytes):
    """ Split raw multipart body by boundary. Returns list of parts (each part includes headers + CRLF + body). """
...
def parse_part_headers_and_body(part: bytes):
    """ Given a multipart part (headers CRLF body), return (headers_dict, body_bytes).
    headers_dict keys are lower-case. """
    ...
    # Remove exactly one structural CRLF from end of body (if present)
    # Assumes common \r\n; preserves content endings like trailing \n
    if body.endswith(b'\r\n'):
        body = body[:-2]
...
def extract_filename_from_headers(headers: dict):
    """ Look for content-disposition header and extract filename param. """
...
def get_http_raw_body_bytes(pkt):
    """ Attempt multiple strategies to obtain raw HTTP body bytes from a pyshark packet:
      - http.file_data (if present) -- hex
      - http.raw_header/body fields (if available)
      - tcp.payload (hex) as a fallback
...
def extract_and_save_multipart(body_bytes: bytes, content_type_header: str, outdir: Path, pkt_info: dict):
    """ Given raw body bytes and the Content-Type header, attempt to parse multipart/form-data
    and save file parts (those with filename=) into outdir. Returns list of dicts for each saved file.
    pkt_info: dict with src/dst/proto metadata to include in the record. """
    saved = []
...
def export_http_objects_from_pcap(pcap_path, outdir):
    """ Extracts files using tshark's internal mechanisms. """
    cmd = ["tshark", "-r", pcap_path, "--export-objects", f"http,{outdir}", '-q']
...
def process_http_packet(pkt, host: str, http_objects_outdir: Path, report: dict, totals: dict, saved_files: list,
    ...
    """ Process a single packet that has an HTTP layer.
        - Tries to detect method, large uploads, extract files from multipart/form-data requests,
        save files referenced by Content-Disposition on responses, and produce report entries.
        - Updates totals, saved_files, report, outbound_by_dst in place. """
    try:
        http = pkt.http
    except Exception:
        return
    ...
    # If we saw bytes from host, increment outbound_by_dst aggregator
    # (caller may already do this, but keep here for safety if called elsewhere)
    try:
        if src == host:
...
def extract_multipart_file(data: bytes) -> bytes:
    """ Extract embedded file from multipart/form-data body if present.
    Otherwise, return data unchanged. (Used in post-processing exported files). """
